...
//	private CatchSellerLocal catchSellerLocal;
	...
	private String userProductCookie = "";
	@Inject
	...
	private boolean contry=false;
	
	private Cookie cookie2;
	
	public String getUserProductCookie() {
		return userProductCookie;
	}

	public void setUserProductCookie(String userProductCookie) {
		this.userProductCookie = userProductCookie;
	}
	...
	public void init() {	
		...
		if (userCookies != null && userCookies.length > 0) {
			for (int i = 0; i < userCookies.length; i++) {
				...
				if (userCookies[i].getName().equals("tehcpr")) {
					// System.err.println(userCookies[i]);
					userProductCookie = userCookies[i].getValue();
					//break;
				}
			}
		}
	...
	public HomeEntity findHomeCatImageByName(String name) {
         try {
			return homeServiceLocal.findHomeEntityByName(name);
		} catch (Exception e) {
...
//	}
    ...
    public List<HomeEntity> findAllSlide(){
		try {
			return homeServiceLocal.findHomeEntityByNameList("C");
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
			return null;
...
//		}
	...
	public String findUserByToken() {
		try {
			...
			this.mobile = session.getAttribute("mobile").toString();
			 System.err.println("*******************************************");
			 System.err.println("*******************************************");
			 System.err.println(this.mobile);
			 System.err.println("*******************************************");
			 System.err.println("*******************************************");
	...
	public void addCook(ComponentSystemEvent event){
		if(userProductCookie.equals("") || userProductCookie == null) {
		HttpServletResponse response=(HttpServletResponse) FacesContext.getCurrentInstance().getExternalContext().getResponse();
		UUID uuid=UUID.randomUUID();
		this.userProductCookie=uuid.toString();
		cookie2 = new Cookie("tehcpr", userProductCookie);
		cookie2.setPath("/");
